...
public interface SessionDetailsRepository extends JpaRepository<SessionDetails, Integer>{
	...
	@Query(value="select s.session_id, s.session_link, s.session_type, s.session_date, s.session_time, s.session_duration, s.slots_available from session_details s join user_session us on s.session_id=us.session_id where us.user_id=:userId and us.session_id=:sessionId",nativeQuery = true)
