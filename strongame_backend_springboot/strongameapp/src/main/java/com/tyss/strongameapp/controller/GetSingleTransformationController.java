 ...
 * Get single transformation controller is to fetch the details of a transformation.
 *
 */
@Slf4j
...
public class GetSingleTransformationController {
	...
	public ResponseEntity<ResponseDto> getTransformationById(@PathVariable int transformationId, @PathVariable int userId) {
		SingleTransformationDto transformationDto=transformationService.getTransformationById(transformationId,userId);
		ResponseDto responseDTO = new ResponseDto();
		// create and return ResponseEntity object
		if (transformationDto.getCases().equalsIgnoreCase("")==false) {
