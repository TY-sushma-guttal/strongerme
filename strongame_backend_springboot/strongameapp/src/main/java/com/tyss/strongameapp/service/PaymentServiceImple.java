...
/**
 * PaymentServiceImple is implementation class which is used for payment management.
...
//	@Value("${algorithm}")
	...
	private static String generatedSignature;
...
//	@Value("${keysecret}")
	...
	public boolean verifySignature(RazorpayResponse data) {
		...
		try {
			generatedSignature = calculateRFC2104HMAC(data.getRazorpay_order_id() + "|" + data.getRazorpay_payment_id(),keySecret);

		} catch (SignatureException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		System.out.println(data.getRazorpay_signature());
		if (generatedSignature.equals(data.getRazorpay_signature())) {
			return true;
		}else {
			return false;
		}
